...
class VRuntime {
	...
	{   
		...
		for(i=0;i<MAX_ALLOC_NUM;i++)
		   if(mmu().alloc(i)->flag==LIST_NOT_INIT)
		     break;
		return i;     
	}
	...
	{   INT i=mmu().searchPos();
		while(i<MAX_ALLOC_NUM){
		   if(mmu().alloc(i)->end>=start && mmu().alloc(i)->start<=start)
		     return i;
		   i=mmu().alloc(i)->next;
		}  
		return i;
	...
	{   INT i=mmu().searchPos();
		while(i<MAX_ALLOC_NUM){
		   if(mmu().alloc(i)->start<=end && mmu().alloc(i)->end>=end)
		     return i;
		   i=mmu().alloc(i)->next;
		}
		return i;  
...
INT VRuntime<MMUClass, REGClass, INSTRClass>::_findfreeblock(INT pagenum){	   	
   	int i,j=-1,best_num=mmu().allocLimit();
   	if(mmu().allocLimit()!=(UINT)-1)
   	    best_num = mmu().allocLimit()/ADD_PAGE_SIZE;
   	else
   	...
   	while(i<MAX_ALLOC_NUM)
   	{
   		if(mmu().alloc(i)->page_num>=pagenum)
   		  if((mmu().alloc(i)->page_num-pagenum)<=best_num){
   		       best_num=mmu().alloc(i)->page_num-pagenum;
   		       j=i;
   		  }
   		i=mmu().alloc(i)->next;   	   
   	}
...
void VRuntime<MMUClass, REGClass, INSTRClass>::setReturn(WORD data,WORD err) {
	if(err == 0)
	{
	   reg().setGPR(INDEX_REG_V0,data);
	   reg().setGPR(INDEX_REG_A3, 0);
	}
	else
	{
	   reg().setGPR(INDEX_REG_V0,err);
	   reg().setGPR(INDEX_REG_A3, 1);
	}
}
...
UINT VRuntime<MMUClass, REGClass, INSTRClass>::uclibc_syscall() {
	...
	switch(id) {
        ...
        case __NR_readv: ret =  _vrtreadv();break;
        case __NR_writev: ret =  _vrtwritev();break;
...
template<class MMUClass, class REGClass, class INSTRClass>
UINT VRuntime<MMUClass, REGClass, INSTRClass>::_vrtreadv() {
	AppFatal((0), ("VRT error: readv system call is not verify yet."));
	WORD ret;
	INT fd = getArgWord(0);
	struct iovec *vector = (struct iovec *)getArgWord(1);
	INT count = getArgWord(2);
	struct iovec temp_buf;
    char buf[count]; //???block_size*count
    temp_buf.iov_base = buf;
    temp_buf.iov_len = count; //????block_size*count
	ret = readv(fd, &temp_buf,count);
    if(errno == 0)
    	mmu().writeBlock((const ADDR)vector->iov_base,(const BYTE *)temp_buf.iov_base,(INT)vector->iov_len);
	setReturn((WORD) ret, errno);
	return ESTATUS_NORMAL;
}

template<class MMUClass, class REGClass, class INSTRClass>
UINT VRuntime<MMUClass, REGClass, INSTRClass>::_vrtwritev() {
	AppFatal((0), ("VRT error: writev system call is not verify yet."));
	WORD ret;
	INT fd = getArgWord(0);
	struct iovec* vector = (struct iovec *)getArgWord(1);
	INT count = getArgWord(2);
    ...
    temp_buf.iov_base = buf;
    temp_buf.iov_len = count; //????block_size*count
    mmu().readBlock((const ADDR)vector->iov_base,(BYTE *)temp_buf.iov_base,(INT)vector->iov_len);
    ret = writev(fd, &temp_buf, count);
    setReturn((WORD) ret, errno);
	return ESTATUS_NORMAL;
}  
...
UINT VRuntime<MMUClass, REGClass, INSTRClass>::_vrtmmap(){
	...
	if(ret!=-1 && ret !=0)	 
	{
	    ...
	    //allocate new page 
	    a=mmu().alloc(k)->start; 
	    if(mmu().alloc(k)->page_num==pagenum)
	    {
	       if(mmu().alloc(k)->start==mmu().allocStart())
	          mmu().allocStart(mmu().alloc(k)->start+pagenum*ADD_PAGE_SIZE);
		   mmu().alloc(k)->flag=LIST_NOT_INIT;
		   i=mmu().alloc(k)->prev;
		   if(i!=MAX_ALLOC_NUM)
		   		mmu().alloc(i)->next=mmu().alloc(k)->next;
		   i=mmu().alloc(k)->next;
		   if(i!=MAX_ALLOC_NUM)
		   		mmu().alloc(i)->prev=mmu().alloc(k)->prev;
		   if(mmu().searchPos()==k)
		      mmu().searchPos(mmu().alloc(k)->next);		   
	    }
	    else
	    {	       
	       mmu().alloc(k)->start+=pagenum*ADD_PAGE_SIZE;
	       mmu().alloc(k)->page_num-=pagenum;
	       if(mmu().alloc(k)->start>mmu().allocStart())
	          mmu().allocStart(mmu().alloc(k)->start);	
 	    }
...
UINT VRuntime<MMUClass, REGClass, INSTRClass>::_vrtmunmap(){
	...
	if(startnum==MAX_ALLOC_NUM && endnum==MAX_ALLOC_NUM)
	{
    	...
    	mmu().alloc(i)->flag=LIST_USED;
    	mmu().alloc(i)->start=data_start;
    	mmu().alloc(i)->page_num = pagenum;
    	mmu().alloc(i)->end = data_end;
    	mmu().alloc(i)->prev = _find_most_end_list(mmu().alloc(i)->start);
    	mmu().alloc(i)->next = _find_most_start_list(mmu().alloc(i)->end);
    	//printf("%d\n",mmu().alloc(1)->flag);
    	if(mmu().alloc(i)->prev!=MAX_ALLOC_NUM){
    		j=mmu().alloc(i)->prev;
    		mmu().alloc(j)->next=i;
    	}
    	if(mmu().alloc(i)->next!=MAX_ALLOC_NUM){
    		j=mmu().alloc(i)->next;
    		mmu().alloc(j)->prev=i;
    	}	
	...
	{
		if(mmu().alloc(startnum)->end>data_start)
		     pagenum = (data_end-mmu().alloc(startnum)->end)/ADD_PAGE_SIZE;
		mmu().alloc(startnum)->page_num += pagenum;
		mmu().alloc(startnum)->end = data_end;
		//printf("%x,%x,%d\n",mmu().alloc(startnum)->start,mmu().alloc(startnum)->end,startnum);
	}
	else if(startnum==MAX_ALLOC_NUM && endnum!=MAX_ALLOC_NUM)
	{
		if(data_end>mmu().alloc(endnum)->start)
		   pagenum= (data_start-mmu().alloc(endnum)->start)/ADD_PAGE_SIZE;
		//printf("aaa,%d\n",mmu().alloc(0)->next);
		mmu().alloc(endnum)->page_num += pagenum;
		mmu().alloc(endnum)->start = data_start;
		//printf("bbb,%d\n",mmu().searchPos());
	}
	else
	{
		if(startnum!=endnum){
			if(data_start<mmu().alloc(startnum)->end)
		   		pagenum-=(data_start-mmu().alloc(startnum)->end)/ADD_PAGE_SIZE;
			if(data_end>mmu().alloc(endnum)->start)
		   		pagenum-=(data_end-mmu().alloc(endnum)->start)/ADD_PAGE_SIZE;   
			mmu().alloc(startnum)->page_num += pagenum;
			mmu().alloc(startnum)->page_num += mmu().alloc(endnum)->page_num;
			mmu().alloc(startnum)->end = mmu().alloc(endnum)->end;
			mmu().alloc(startnum)->next =mmu().alloc(endnum)->next;
			i=mmu().alloc(endnum)->next;
			if(i!=MAX_ALLOC_NUM)
				mmu().alloc(i)->prev=startnum;
			mmu().alloc(endnum)->flag=LIST_NOT_INIT;
		}
	}
	INT err = 0;
	INT ret = 0;
